...
var self = {
    ...
    getIssue: function(uri, issue) {
        return new Promise(function (fulfill) {
            self.jiraApi.findIssue(issue, function (error, issueResult) {
                ...
                if (issueResult.fields.parent) {
                    parentIssue = {
                        key: issueResult.fields.parent.key,
                        summary: issueResult.fields.parent.fields.summary
                    }
                }
    ...
    /**
     * Intercept JIRA app for Slack.
     *
     * @param {object} jiraIssueResult
     * @param {string} jiraUrl
     * @param {object} message
     *
     * @returns {object}
     */
    interceptJiraApp: function (jiraIssueResult, jiraUrl, message) {
        var updateMessage = '',
            updateAttachmentText = message.attachments[0].pretext.replace(
                '<',
                '*' + message.attachments[0].title + '* <'
            ).replace('<', '(<').replace('>', '>)');

        if (jiraIssueResult.parent) {
            updateMessage = 'Change in: <' + jiraUrl +
                jiraIssueResult.parent.key + '|' +
                jiraIssueResult.parent.key + ' - ' +
                jiraIssueResult.parent.summary + '>';
        } else {
            // Change is on parent itself.
            updateMessage = updateAttachmentText;
            updateAttachmentText = '';
        }

        var updateAttachment = [{
            "mrkdwn_in": ["text"],
            "text": updateAttachmentText
        }];
        if (message.attachments[0].text) {
            updateAttachment.push({
                "mrkdwn_in": ["text"],
